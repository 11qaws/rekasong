...
- production Worker: `https://rekasong-session.11qaws.workers.dev`. 공개 앱의 Protocol v2 실제 경로다.
- 기존 staging Worker `https://rekasong-session.11qaws-test.workers.dev`는 현재 legacy snapshot만 반환한다. 별도 업그레이드 전까지 Protocol v2 OBS 시험 주소로 사용하지 않는다.
- 반복·장애·부하 테스트는 로컬/격리 Worker에서 수행한다. production 실제 CEF 인수 시험은 사용자 명시 승인, 전용 test profile, 방송 OFF 재확인, bounded 단일 session일 때만 실행한다.
...
npm run test:obs:v2:cef-scene-transition
```
...
1. 시작·중간·끝을 구분할 수 있는 click marker가 든 5분 기준 트랙을 한 곡으로 재생한다. 10초 주기라면 `0초`와 `300초`를 모두 포함한 31개 marker가 필요하다. 10분 확장판은 별도의 stress 진단으로 실행한다.
...
5. 시작과 끝의 offset 차이로 drift를 판정한다. endpoint가 실제 marker로 직접 관측되지 않은 창은 추정값만으로 통과시키지 않는다.

초기 프로젝트 판정 기준 제안:

- marker 누락·중복 및 청취 가능한 dropout: 0회
- 보정 후 마이크↔MR 고정 offset: 목표 `±20ms` 이내
- 한 곡(최대 5분) 시작↔끝 상대 drift: `10ms` 이내
- click 간격 오차 p95: `5ms` 이내

곡마다 새 재생 기준점을 만들되 OBS route는 유지한다. 30초마다 위치를 비교할 수는 있지만 이는 관찰·기록일 뿐이다. 곡 중간에는 측정값을 이유로 자동 seek·restart·속도 보정을 하지 않으며, 10분 stress 결과는 연결·재생 차단 조건으로 사용하지 않는다.
...
- [OBS] **완료(2026-07-23):** 공개 v0.2.26 player의 302.5초 실제 CEF 재생에서 lifecycle 즉시 전달과 30초 position 10회를 직접 계수했다. 최소 수신 간격 `30,025ms`, wall 오차 `132ms`, candidate 전환·disconnect/reconnect·unsafe route 0이었고 관측으로 인한 seek·restart·속도 변경·재연결은 없었다. 실행 전·중·후 방송·녹화 OFF와 원본 설정 exact restore도 확인했다.
...
5. 완료: v2 route split, heavy graph/font 제외, raw/gzip budget, OBS 10초 heartbeat 무렌더, active/prefetch 64MiB cap과 실제 OBS CEF 60분 soak를 통과했다. Dashboard는 1,000곡 window와 로컬 Blob 최근 5개·합계 256MiB 상한을 가지며, v0.2.28 production-browser 30곡 반복에서 25개 선회수·unmount 뒤 30/30 회수·저장 Blob URL 0·Worker 요청 0을 확인했다. OBS 60분 실측은 wall/media 3,600,150/3,600,000ms, player/OBS 후보 1/1, unsafe route와 identity 전환 0건, renderer private 14.8MiB·working set 약 33.5~33.6MiB였다.
