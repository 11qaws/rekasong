...
1. 시작·중간·끝을 구분할 수 있는 click marker가 든 5분 기준 트랙을 한 곡으로 재생한다. 10분 확장판은 별도의 stress 진단으로 실행한다.
...
곡마다 새 재생 기준점을 만들되 OBS route는 유지한다. 곡 중간에는 측정값을 이유로 자동 seek·restart·속도 보정을 하지 않으며, 10분 stress 결과는 연결·재생 차단 조건으로 사용하지 않는다.
