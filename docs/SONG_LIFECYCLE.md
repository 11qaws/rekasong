...
| `finishing` | Speaker 스킵의 `ended`, 또는 OBS 곡 경계의 강한 정지를 기다림 | 스킵 중 / OBS 정지 확인 중 |
| `discarding` | 쓰레기통으로 stop/unload를 요청한 뒤 확인을 기다림 | 취소 중 |
| `stop_unconfirmed` | OBS STOP 전송 또는 강한 정지 증거가 지연됨. 현재 곡과 예약한 다음 곡을 모두 보존함 | 정지 확인 필요 |
...
stateDiagram-v2
    ...
    finishing --> completed: Speaker ended 또는 OBS strong-stop 확인
    playing --> completed: Speaker 자연 ended 확인
    playing --> finishing: OBS 자연 ended 뒤 strong-stop 요청
    ...
    finishing --> stop_unconfirmed: OBS strong-stop 확인 지연
    discarding --> stop_unconfirmed: OBS strong-stop 확인 지연
    stop_unconfirmed --> finishing: 완료 STOP 다시 요청
    stop_unconfirmed --> discarding: 폐기 STOP 다시 요청
...
| `stop_unconfirmed` | 현재 곡과 다음 곡 예약을 보존하고 `정지 다시 요청`을 제시 | 실패 이력 편입, 다음 곡 시작, 경로 자동 절단 |
...
2. **명령 중인 곡을 숨기지 않는다.** `finishing`, `discarding`, `stop_unconfirmed`, `starting`은 짧더라도 사용자에게 보여야 한다.
3. **상태별로 가능한 행동만 노출한다.**
   - `finishing`: 쓰레기통만 허용
   - `discarding`: 중복 조작을 막고 취소 확인 상태를 표시
   - `stop_unconfirmed`: 일반 재생을 잠그되 같은 run의 STOP 재요청과 명시적 긴급 정지를 제시
