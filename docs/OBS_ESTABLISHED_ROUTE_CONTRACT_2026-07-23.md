...
스킵·자연 종료·현재 곡 폐기는 모두 다음 순서를 사용한다.

1. Dashboard는 현재 `entryId/runId`에 묶인 STOP을 한 번 요청한다.
2. 현재 곡 제목과 선택한 다음 곡은 그대로 유지하고 `OBS 정지 확인 중`을 표시한다.
3. 동일 run의 strong-stop relay 또는 authoritative snapshot을 확인한다.
4. 그 뒤에만 현재 곡을 완료 이력으로 보내거나 폐기하고, 예약된 다음 곡을 0초에서 LOAD한다.

일반 ACK, `paused`, 자연 `ended`, 과거 run의 `stopped`는 3단계를 대신하지 않는다. 8초 안에 증거가 없거나 STOP 전송이 실패하면 `정지 확인 필요`로 바꾸되 route와 목록을 자동으로 절단하지 않는다. 사용자가 `정지 다시 요청`을 누를 때만 같은 run의 STOP을 재발행한다. Worker snapshot과 relayed event가 같은 증거를 함께 보내더라도 완료/다음 LOAD는 한 번만 수행한다.
...
- `정지 확인 필요`에서는 현재 곡과 다음 곡을 보존하고, 먼저 `정지 다시 요청`을 안내한다. 검증 실패만으로 완료 이력을 만들거나 Speaker로 자동 전환하지 않는다.
