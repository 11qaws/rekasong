...
- G6는 물리 스피커 출력과 FIFINE 마이크 입력을 분리 track으로 10분 기록했다. marker와 jitter는 통과했고 장시간 drift를 수치화했다. endpoint-inclusive 31-marker/300초 재분석에서 edge 최악 `9.753ms`는 통과했지만 linear-fit 최악 `10.408ms`는 경계를 `0.408ms` 넘었고, 시작 offset도 실패했다.
...
| G6 보컬↔MR 싱크 | **장시간 측정 완료·시작 offset 실패·5분 drift 경계/재검 필요** | `2026-07-22 21-55-45.mkv`: track 2 MR, track 3 FIFINE 마이크, 60/60 marker | 5분 edge `9.753ms`, linear-fit `10.408ms`, offset `43.25ms`. 같은 clock 경로로 재검증 |
