...
현재 실제 장비 증거는 G4를 통과했고 G6까지 측정했다. 다만 G6는 현재의 온보드 스피커 출력+USB FIFINE 마이크 입력 조합에서 10분 상대 drift가 기준을 넘어 **수용 실패**다. 비공개 방송 결과 G5와 다른 monitoring 장치 조합은 별도 관문이다.
...
- 실제 녹화 artifact에서 기준 패턴을 검출해 G4를 통과했다. 물리 스피커→마이크 경로의 10분 분리 track도 분석했지만 drift 기준을 넘었다. 플랫폼 결과물은 실행하지 않았다. 따라서 현재 상태를 `카라오케 싱크 확인 완료` 또는 `플랫폼 송출 확인 완료`라고 부르면 안 된다.
...
- 이 통과는 실제 OBS CEF 장시간 경로·재생·자원 안정성의 증거다. ingest 결과물은 G5가 필요하고, 마이크↔MR은 별도 G6 물리 측정에서 수용 실패했다.
...
- 그러나 첫 5 cycle↔마지막 5 cycle 상대 drift가 `15.5 ms`, 선형 회귀 drift가 `17.32 ms/590초`, 중앙 offset이 `43.25 ms`여서 drift `≤10 ms/10분`, 보정 후 offset `±20 ms` 기준을 통과하지 못했다.
- Browser Source의 `+69 ms` OBS sync offset은 상대 지연을 약 `82–84 ms`로 악화시켰고 drift를 해결하지 못해 `0 ms`로 복원했다.
- 판정은 **G6 측정 완료·수용 실패**다. 실패는 route를 차단하거나 재생을 끊는 조건이 아니다. 같은 audio clock 장치 또는 저지연 performer monitoring 경로를 설계한 뒤 같은 fixture로 재검증한다.
...
| 스트리머 마이크와 반주 싱크가 방송 내내 유지 | **현재 장치 조합 실패** | 60/60 marker·jitter 통과, 10분 drift `15.5–17.32 ms`와 offset `43.25 ms`로 G6 기준 초과 |
...
- **현재 장치 구성 측정 완료·수용 실패:** `2026-07-22 21-55-45.mkv`, MR track 2·마이크 track 3
- marker 60/60, 누락·중복 0 — 통과
- jitter p95 `1.832 ms` — `≤5 ms` 통과
- 시작·끝 구간 상대 drift `15.5 ms`, 선형 회귀 `17.32 ms/590초` — `≤10 ms/10분` 실패
...
6. 완료(측정)·수용 실패: 10분 mic/MR fixture와 상호상관 분석기로 현재 장치 G6를 실행했다. route 연속성·marker·jitter는 통과했지만 drift와 offset은 기준을 넘었다.
7. 같은 audio clock 장치 또는 저지연 performer monitoring 경로를 설계하고, 검증 실패가 established OBS route를 차단하지 않는 advisory UX를 고정한 뒤 G6를 재실행한다.
