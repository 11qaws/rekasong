...
- 실제 녹화 artifact에서 기준 패턴을 검출해 G4를 통과했다. 물리 스피커→마이크 경로의 10분 분리 track과 31-marker/300초 창도 분석했지만, 시작 offset이 기준을 넘고 5분 linear-fit drift도 경계를 약 `0.4ms` 넘었다. 플랫폼 결과물은 실행하지 않았다. 따라서 현재 상태를 `카라오케 싱크 확인 완료` 또는 `플랫폼 송출 확인 완료`라고 부르면 안 된다.
...
- 이 통과는 실제 OBS CEF 장시간 경로·재생·자원 안정성의 증거다. ingest 결과물은 G5가 필요하고, 마이크↔MR은 별도 G6에서 시작 offset 실패·5분 drift 경계/재검 필요 상태다.
...
- 첫 5 cycle↔마지막 5 cycle 상대 drift가 `15.5 ms`, 선형 회귀 drift가 `17.32 ms/590초`, 중앙 offset이 `43.25 ms`였다. 기존 10분 기준과 보정 후 offset `±20 ms` 기준에는 실패했다. 제품 단위인 5분을 31-marker/300초 창으로 직접 재분석한 결과 edge drift 최악 `9.753 ms`는 통과했지만 linear-fit drift 최악 `10.408 ms`는 `0.408 ms` 초과했다. 8kHz 교차 분석도 `9.825 / 10.428 ms`로 재현됐다.
- 30초 상대 변화는 중앙 `1.047 ms`, p95 `2.486 ms`, 최악 `3.471 ms`였다. 이 cadence는 관찰용이며 곡 중 seek·restart·playback-rate 보정이나 route 전이를 만들지 않는다.
...
### 1.6 2026-07-23 source refresh·OBS 재시작 복구 잠금 수정

- 원인을 상태 단위로 다시 추적했다. 같은 페이지의 WebSocket 재접속은 `playerInstanceId`가 같아 기존 route를 자동 복원하지만, Browser Source refresh와 OBS 재시작은 새 페이지·새 ID를 만든다. Worker가 사라진 이전 ID의 정지 ACK만 기다리면 새 플레이어가 정상이어도 `emergency_stopping`에서 영구 해제되지 않았다.
- 기존 Dashboard의 완전 초기화는 `emergency_stop_dispatched` ACK만 기다린 뒤 제어 socket을 재생성했다. 실제 lease가 inactive로 수렴했는지는 기다리지 않아 성공 안내 뒤에도 같은 잠금이 다시 보일 수 있었다.
- 수정 뒤 일반 emergency stop은 이전과 같이 정확한 대상의 strong-stop 증거만 허용한다. 사용자 확인을 받은 완전 초기화만 현재 연결된 모든 출력의 정지 ACK를 기다린 뒤, 이미 사라진 이전 target은 `output_inactive + recoveryOverride`로 미확인 표시하면서 route 잠금을 해제한다. ACK 전에 사라진 대기 출력도 미확인으로 기록해 영구 대기를 만들지 않는다.
...
| source refresh·OBS 재시작으로 플레이어 ID가 바뀐 뒤 영구 잠기지 않음 | 로컬 실제 브라우저+Worker 확인 | 명시적 완전 초기화가 연결된 출력 정지 뒤 이전 target을 미확인으로 해제하고 Speaker로 복귀; 실제 OBS live-session 변형은 남음 |
...
| 검증 | 결과 |
|---|---|
| 전체 테스트 | v0.2.17 최신 후보 696/696 통과 |
| 완전 초기화 집중 테스트 | 262/262 통과 |
...
- hide/show는 실제 통과. scene 전환은 남음
- source refresh·OBS 재시작은 설정 보존만 실제 확인. 유효한 live-session URL에서 완전 초기화→무음→명시적 OBS 재선택을 확인해야 함
...
- **현재 장치 구성 장시간 측정 완료·시작 offset 실패·5분 drift 경계/재검 필요:** `2026-07-22 21-55-45.mkv`, MR track 2·마이크 track 3
...
6. 완료(장시간 및 5분 재분석): 10분 mic/MR fixture와 상호상관 분석기로 현재 장치의 stress drift를 수치화하고 endpoint-inclusive 5분 rolling 창을 재판정했다. route 연속성·marker·jitter와 5분 edge 통계는 통과했지만 시작 offset은 실패했고 5분 linear-fit은 약 `0.4ms` 초과했다.
...
9. 완료(자동·로컬 브라우저): source refresh·OBS 재시작으로 새 player ID가 생긴 경우의 완전 초기화, 사라진 출력 미확인 표시, Speaker 복귀, 무자동재생과 새 route 재선택을 고정했다. 실제 OBS live-session 변형은 수동 관문으로 남는다.
10. 일반 Chrome READY 10분과 실제 OBS CEF 60분 soak는 완료했다. 100/500/1,000회 부하 예산을 이어서 자동화한다.
11. 전체 i18n·pseudo-locale를 완료한다.
