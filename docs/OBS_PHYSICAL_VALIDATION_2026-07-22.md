...
>
> 추가 확립 lease 중복 후보 검증: 공개 `0.2.36` Dashboard / 실제 OBS CEF + production Worker 통과
>
...
## 14. v0.2.30 무재생 실제 OBS 연결 점검 — 2026-07-23

- 전용 시험 profile·scene에서 실제 OBS를 열어 방송과 녹화 타이머가 모두 `00:00:00`이고 시작 버튼 상태임을 확인했다. LOAD, PLAY, recording, streaming은 실행하지 않았다.
- Browser Source는 public `/widget`과 production Worker protocol 2를 가리켰고 `Control audio via OBS`가 켜져 있었다. source는 enabled/unmuted, volume 1이며 monitoring은 Monitor and Output이었다.
- OBS browser child process에서 production Worker DNS 주소로 established TCP 연결 하나가 있었다. credential을 출력하지 않는 단일 read-only 404 probe까지 합쳐, 저장된 URL만 맞는 것이 아니라 현재 CEF와 세션 인증 경로가 실제로 살아 있음을 확인했다.
- 이 점검은 무재생 연결 확인이므로 v0.2.30 코드로 mixer meter, audible monitoring, 녹화 PCM, offset/drift를 반복 측정한 것은 아니다. 출력별 음소거 복원 변경은 자동 회귀 `33/33`과 전체 `734/734`로 검증했으며, 물리 관문은 이전 G3/G4 증거와 최종 사용자 청취·G5·같은-clock G6를 구분해 유지한다.
- 30초 observation은 상대 오차를 기록할 뿐이다. 곡 중간에 위치나 속도를 강제 보정하지 않고 route와 media graph를 유지한다. 곡이 자연 종료된 뒤 다음 곡의 새 run을 0초에서 시작할 때만 동기 기준을 다시 세운다.

## 15. v0.2.36 공개 Dashboard → 실제 OBS 3곡 경계 — 2026-07-23

- 환경: 공개 Dashboard `https://11qaws.github.io/rekasong/`, production Worker, OBS 30.2.0 / obs-browser 2.23.5, 전용 `Rekasong_Local_Record_Test_20260722 / Scene / Browser`, `Control audio via OBS=true`.
- 시험 시작 전과 진행 중 CEF player는 정확히 하나였고 runtime attestation은 `streaming=false`, `recording=false`였다. Dashboard용 자동 브라우저만 음소거했으며 OBS Browser Source의 media 경로는 그대로 사용했다.
- A곡 스킵의 frame 순서는 STOP `24` → 동일 run exact strong-stop `28` → B곡 LOAD `30`이었다. Dashboard는 strong-stop 전까지 A곡 제목과 정지 확인 의도를 보존했다.
- B곡 자연 종료의 frame 순서는 ended evidence `51` → STOP `53` → exact strong-stop `57` → C곡 LOAD `60`이었다. 자연 종료가 다음 LOAD의 직접 허가가 되지 않았고, 별도 source detach 증거 뒤에만 다음 곡을 시작했다.
- C곡 폐기는 STOP `78` → exact strong-stop `81` 뒤에만 최종 idle로 확정됐다. page error와 console error는 각각 0건이었고 session은 HTTP 410이었다.
- 합격 결과는 `D:\Agents\rekasong\Codex\artifacts\obs-dashboard-boundary-v0236-20260723-145413\status.json`에 credential 없이 보존했다. 앞선 두 중단 실행은 닫힌 설정 상세의 checkbox와 React controlled checkbox를 직접 조작한 harness 결함이었으며, 두 실행 모두 session HTTP 410으로 정리했다.
- 시험 뒤 Browser Source URL은 원본 길이 `214`·SHA-256 `e654020bc4e70f0faf7bc5f5e5bf8672891ad461126030ecd254093873e07a2d`와 exact match로 복원했다. Browser source 1개, visible scene item 1개, `Control audio via OBS=true`, handoff와 `.next`/`.rollback` 잔여 0건을 확인했다.
- OBS는 원래 source로 다시 열어 두었다. UI는 `Start Streaming`·`Start Recording`, 두 타이머 `00:00:00`이고 완료 run 로그 `2026-07-23 14-54-51.txt`의 Streaming/Recording Start·Stop은 모두 0건이다. 실제 방송과 녹화는 시작하지 않았다.

이 검증은 v0.2.36의 사용자 경로 전체가 스킵·자연 종료·폐기에서 같은 strong-stop 장벽을 지킨다는 실제 구성 증거다. 30초 cadence는 이 경계를 만들거나 오디오를 보정하지 않으며 계속 표시 기준 갱신과 관찰만 수행한다. 사용자 monitoring 청취, 플랫폼 ingest/VOD G5, 같은-clock performer monitoring G6는 별도 관문으로 남는다.

## 16. v0.2.36 실제 OBS 확립 lease에 두 번째 후보 진입·이탈 — 2026-07-23

- 환경은 공개 Dashboard, production Worker, 실제 OBS 30.2.0 / obs-browser 2.23.5, 전용 `Rekasong_Local_Record_Test_20260722 / Scene / Browser`, `Control audio via OBS=true`다. 자동 Dashboard 브라우저만 음소거했고 실제 OBS CEF media 경로는 그대로 사용했다.
- 실제 OBS player 하나가 A곡을 재생해 `lease.status=audible`과 `confirmedPlayback.status=playing`을 확정한 뒤 같은 세션의 두 번째 OBS-compatible player를 연결했다. player와 eligible 후보는 각각 `1→2`가 됐지만 lease target은 실제 OBS에서 바뀌지 않았다.
- 기존 player route attachment는 `ready`, 추가 player의 서버 상태는 `standby`였다. 추가 player의 audio는 `paused=true`, `currentSrc=''`, `src` 없음, `currentTime=0`이었고 LOAD·PLAY·PAUSE·SEEK·VOLUME·STOP 수신은 0건이었다. Dashboard도 후보 진입·이탈 동안 media command를 보내지 않았으며 `OBS 송출 중`과 A곡 제목을 유지했다.
- 추가 player를 닫자 동일 player identity 하나로 돌아왔고 audible lease와 playing run이 계속 유지됐다. 이후 A 스킵 STOP `26` → strong-stop `30` → B LOAD `32`, B 자연 종료 ended `53` → STOP `54` → strong-stop `60` → C LOAD `62`, C 폐기 STOP `80` → strong-stop `84`도 통과했다.
- page error와 console error는 0건, session은 HTTP 410이다. credential을 제외한 결과는 `D:\Agents\rekasong\Codex\artifacts\obs-dashboard-duplicate-v0236-20260723-152400\status.json`에 보존했다.
- 시험 전·중 OBS runtime은 `streaming=false`, `recording=false`였다. 완료 로그 `2026-07-23 15-22-55.txt`의 Streaming/Recording Start·Stop은 모두 0건이다.
- 종료 뒤 scene collection을 시험 전 백업과 전체 파일 SHA-256 `6c56fe4804fa0fc65cf50fc65fa64525562a4ef8d65152681bee0f0fe94050d0`으로 바이트 exact match 복원했다. Browser URL은 길이 `214`·SHA-256 `e654020bc4e70f0faf7bc5f5e5bf8672891ad461126030ecd254093873e07a2d`, source 1개, visible item 1개, `Control audio via OBS=true`이고 handoff와 harness process 잔여는 0건이다. OBS는 원래 source로 다시 열어 `Start Streaming`·`Start Recording`, 두 타이머 `00:00:00` 상태로 두었다.

이 검증은 “확립된 OBS 경로는 새 후보 탐색 조건이 흔들려도 기존 정확한 lease를 우선 보존한다”는 운영 관문을 실제 CEF에서 닫는다. 새 OBS 출력을 처음 고를 때 정확히 한 후보를 요구하는 엄격함은 그대로이고, 사용자 monitoring 청취·플랫폼 G5·같은-clock performer monitoring G6는 별도 관문이다.
