...
- 자동 검증: adapter 연결 우선, 초기 active/visible 미관측과 명시적 false 분리, Worker OBS hello 단계 즉시 재연결 복구, 살아 있는 playback 상태 재보고, 보고 결과 불명 시 media graph 보존, 장면 telemetry 즉시 관측·graph 보존, Speaker 독립 로컬 재생, OBS 후보 중복 차단.
...
- 실제 OBS 잔여: 사용자 청취, scene 전환·source refresh·OBS 재시작, 비공개 방송 artifact, 장시간 kara­oke drift.
