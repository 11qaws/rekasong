...
| OBS 스킵·자연 종료·폐기 뒤 이전 음원이 멎기 전에 다음 곡을 시작하지 않음 | 동일 run strong-stop 장벽·지연/유실/중복 증거 회귀 + 실제 OBS 3곡 경계 완료 | v0.2.36 공개 배포·artifact exact match·실제 공개 Dashboard/OBS 통과 | 없음 |
...
- 공개 Dashboard, production Worker, 실제 OBS 30.2.0 전용 test profile·scene·Browser Source를 사용했다. CEF player는 정확히 하나였고 runtime은 시험 전·중 `streaming=false`, `recording=false`였다.
- A곡 스킵은 STOP frame `24` → 동일 run exact strong-stop `28` → B곡 LOAD `30` 순서였다. A곡 제목은 정지 확인 중에도 화면에 남았다.
- B곡 자연 종료는 ended evidence `51` → STOP `53` → exact strong-stop `57` → C곡 LOAD `60` 순서였다. 자연 종료 역시 B곡 제목을 strong-stop까지 보존했다.
- C곡 폐기는 STOP `78` → exact strong-stop `81` 뒤에만 최종 idle이 됐다. page error와 console error는 0건이고 session은 HTTP 410으로 종료됐다.
- Browser Source URL은 원본 길이 `214`·SHA-256 `e654020bc4e70f0faf7bc5f5e5bf8672891ad461126030ecd254093873e07a2d`와 exact match로 복원했다. Browser source 1개, `Control audio via OBS=true`, visible source 1개, 임시 handoff와 원자 교체 잔여 0건을 확인했다.
- OBS를 원본 상태로 다시 열어 `Start Streaming`·`Start Recording`, 두 타이머 `00:00:00`을 확인했다. 완료 run 로그 `2026-07-23 14-54-51.txt`의 Streaming/Recording Start·Stop은 모두 0건이다. 실제 방송과 녹화는 시작하지 않았다.
- credential을 제외한 합격 결과는 `D:\Agents\rekasong\Codex\artifacts\obs-dashboard-boundary-v0236-20260723-145413\status.json`에 보존했다. 이 인수는 사용자 monitoring 청취, 플랫폼 ingest/VOD G5, 같은-clock performer monitoring G6를 대신하지 않는다.

### v0.2.36 실제 OBS 확립 lease + 중복 후보 진입·이탈 인수 — 2026-07-23

- 공개 Dashboard와 production Worker가 실제 OBS 30.2.0 CEF player 하나에 A곡을 재생해 `lease.status=audible`, `confirmedPlayback.status=playing`을 확정한 뒤, 같은 player URL을 가진 두 번째 OBS-compatible page를 연결했다.
- player 수와 eligible OBS 후보 수가 `1→2`가 되어도 lease target은 실제 OBS player에서 바뀌지 않았다. 기존 player의 route attachment는 `ready`, 추가 player의 서버 상태는 `standby`였고 Dashboard는 `OBS 송출 중`과 A곡 제목을 계속 표시했다.
- 추가 player는 2초 관찰 동안 audio가 `paused=true`, `currentSrc=''`, `src` attribute 없음, `currentTime=0`이었다. LOAD·PLAY·PAUSE·SEEK·VOLUME·STOP 수신은 모두 0건이고, 후보 진입·이탈 동안 Dashboard가 보낸 media command도 0건이었다.
...
## 11. v0.2.32 Speaker 기기 일시정지 복구·공개 배포 증거 — 2026-07-23

- `wantsPlayback=true`인 같은 Speaker run의 실제 media만 paused이면 자동 명령 없이 사용자에게 `계속 재생` 행동을 보인다. 명시적 사용자 pause는 일반 pause로 남고, 자연 종료는 기존 auto-next/이력 경계를 사용한다. 페이지 복귀 관측과 늦은 native pause 이벤트 순서가 바뀌어도 같은 의도 분류로 수렴한다.
- 공개 Chrome에서 로컬 WAV를 물리 pause하고 복귀 관측 뒤 행동을 눌렀다. 같은 source와 위치가 보존되고 media가 다시 전진했으며 320px overflow `0`, 행동 높이 `44px`였다. 이 흐름의 session HTTP, WebSocket, 송신 frame과 Worker host 요청은 모두 0이어서 OBS 경로 검사나 서버 연결이 Speaker 복구에 들어오지 않는다.
...
- 30초 cadence는 리모컨 표시와 실제 player 위치를 관찰하는 주기일 뿐이다. 곡 중간 seek·restart·playbackRate·route 전환·WebSocket 재연결을 하지 않고 이전 곡의 strong-stop 뒤 다음 run만 `position: 0`에서 시작한다. 실제 OBS 중복 후보 진입·이탈은 v0.2.36 공개 Dashboard와 production Worker에서 통과했다. 실제 모바일 백그라운드/PiP·물리 출력 장치 청취, 사용자 OBS monitoring 청취, 별도 명시 승인 뒤 G5, 같은-clock/저지연 performer monitoring의 곡 단위 G6는 계속 별도 관문이다.
...
- 방송·녹화 OFF 상태의 실제 OBS CEF 중복 후보 진입·이탈은 v0.2.36 공개 경로에서 통과해 운영 관문을 닫았다.
...
## 15. v0.2.34 Speaker 자연 종료·공개 배포 증거 — 2026-07-23

- 공개 v0.2.33의 6초 WAV 재생은 `0초 → 6초 → ended`와 이력 편입까지 정상 완료됐지만, 같은 evidence observer 안에서 Dashboard가 Speaker STOP을 다시 보내 내부 코드 `observer_reentry`를 토스트로 노출했다. 이는 재생 실패가 아니라 성공한 자연 종료의 중복 정리 결함이었다.
- v0.2.34는 Speaker의 자연 종료를 이미 확인된 물리 정지로 취급한다. Dashboard는 이 경계에서 중복 STOP을 보내지 않고, 로컬 컨트롤러가 observer frame을 벗어난 뒤 같은 `runId`와 `status=ended`를 다시 확인해 source만 정리한다. 새 곡 LOAD가 먼저 시작되면 이전 정리는 취소되므로 다음 곡을 멈추지 않는다.
- OBS 자연 종료는 기존 strong-stop 계약을 유지한다. Speaker 전용 정리 외에 OBS lease, Worker protocol, 방송·녹화 제어, 30초 위치 cadence를 변경하지 않았다. 30초 관찰은 계속 command 0이며 곡 중간 seek·restart·playbackRate·route 재연결을 만들지 않는다.
- 로컬 production-build와 공개 Pages에서 각각 4초 fixture의 자연 종료, idle 전환, 내부 코드 비노출을 확인했다. 두 환경 모두 Speaker 유휴·재생·pagehide 수명·기기 pause 복구·자연 종료의 session HTTP, WebSocket, 송신 frame은 0이었고 공개 검증의 Worker host 요청도 0이었다.
...
## 17. v0.2.36 OBS 곡 경계 strong-stop 장벽 — 2026-07-23

- 기존 OBS 스킵 폴백은 STOP 명령의 동기 반환을 실제 물리 정지처럼 취급해 현재 곡을 먼저 완료하거나 다음 곡 LOAD를 열 수 있었다. 자연 `ended` 역시 source detach와 autoplay 취소를 별도로 증명하지 않은 채 완료 전이로 들어갈 수 있었다.
- 스킵·자연 종료·폐기를 `현재 entry/run에 묶인 STOP → 같은 run의 strong-stop relay 또는 authoritative snapshot → 완료/폐기 확정 → 선택한 다음 곡의 새 run LOAD`로 통일했다. snapshot은 `stopped + paused + sourceDetached + autoplayCancelled + audible=false`를 모두 요구하며 ACK, pause, ended, 과거 run은 완료 근거가 아니다.
- 첫 STOP 전송이 실패하거나 8초 안에 강한 증거가 오지 않아도 current/queue/route를 절단하지 않는다. `정지 확인 필요`와 44px `정지 다시 요청` 행동을 한국어·영어로 표시하며, 사용자가 누를 때만 동일 run의 STOP을 다시 발행한다. 일반 중복 STOP은 계속 한 번으로 합치고 snapshot과 relay가 함께 도착해도 history/LOAD를 한 번만 수행한다.
- 전체 `751/751`, 집중 회귀 `81/81`, lint 신규 오류 0, Worker 문법, production build, pseudo-locale, 30곡 Blob 수명, OBS bundle을 통과했다. 30곡 run은 생성 Blob 30개를 unmount까지 30/30 회수했고 저장된 Blob URL과 Worker 요청은 0이었다.
- build는 Dashboard `382.38kB raw / 104.61kB gzip`, CSS `63.67kB / 11.97kB`, Speaker lazy chunk `8.34kB / 2.85kB`, OBS player `42.66kB / 12.53kB`다.
...
- 첫 Linux 실행은 OS 기본 언어가 영어여서 최초 한국어 UI 기대가 불안정하다는 하네스 결함을 배포 전에 잡았다. smoke는 저장 locale을 한국어로 명시한 뒤 영어 전환·reload를 검증해 Windows와 Linux에서 같은 시작 상태를 사용한다. 앱의 실제 locale 선택 동작은 변경하지 않았다.
