...
export async function searchGroundedWebLyrics(input, apiKey, fetchImpl = globalThis.fetch, {
  ...
  const sourceOrder = requiredCategory === 'namuwiki'
    ? `Search only public NamuWiki pages on namu.wiki. If NamuWiki has no complete exact match, return completeLyricsConfirmed false and no lines.`
    : `Search in this order, continuing only when the earlier tier has no complete match:
1. NamuWiki.
2. Dedicated structured lyric sources with public access, or an official artist/release lyric page.
3. Other subculture sources and discovery databases, especially Touhou Wiki and VocaDB. Follow their cited original source when the database itself does not license or expose the full lyrics.
...
Artist: ${JSON.stringify(input.artist)}`;
  ...
  if (candidate && requiredCategory
    && candidate.discoveryPath[candidate.discoveryPath.length - 1] !== requiredCategory) {
    candidate = null;
  }
  if (!candidate) {
