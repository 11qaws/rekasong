...
test('automatic preparation translates the whole timed source and stops at review required', async () => {
  ...
  assert.deepEqual(phases, [
    ...
    AUTOMATIC_LYRICS_PHASES.TIMING,
...
test('automatic preparation uses verified web translations without calling the model', async () => {
  let translateCalled = false;
  const draft = await createAutomaticLyricsDraft({
    song: { src: candidate.videoId, title: 'Synthetic' },
    candidate: {
      ...candidate,
      sourceKind: 'vocaro_verbatim_lyrics',
      cues: undefined,
      lines: ['original one', 'original two'],
      translations: ['번역 하나', '번역 둘'],
      translationSourceKind: 'vocaro_korean_translation',
      translationSourceTitle: 'Synthetic - 보카로 가사 위키',
      translationSourceUrl: 'https://vocaro.wikidot.com/synthetic',
  ...
  assert.equal(draft.originalText, 'original one\noriginal two');
  assert.equal(draft.translationText, '번역 하나\n번역 둘');
  assert.equal(draft.translationSourceTier, 'trusted_web');
  assert.equal(draft.translationProviderId, 'vocaro_korean_translation');
  assert.equal(draft.translationSourceUrl, 'https://vocaro.wikidot.com/synthetic');
  assert.equal(draft.originalTextLocked, true);
  assert.equal(draft.autoGenerated, false);
});
