...
- 집중 회귀 `262/262`, 전체 `696/696`, lint 신규 오류 0(기존 Gemini escape 경고 2건), Worker·smoke 문법, `git diff --check`, production build를 통과했다. OBS 정적 closure는 raw `383,782B`, gzip `117,547B`, brotli `102,913B`로 예산 안이다. 로컬 production Dashboard smoke는 Speaker 기본, 한·영 reload, 320/375/768/1100px, 3px 금발 선, ntfy 요청 0·HTTP 오류 0을 통과했고 warm DCL `22.2ms`, long task 0, JS heap 약 `7.5MiB`였다.
- release commit `1ca0555a161526614a82e9a5c1ab8a221afca2b6`을 push하고 production Worker `51f9c6e8-60aa-4f74-962a-98ac55eda0be`를 100% 배포했다. Pages workflow `29939145445`는 clean install·696 tests·lint·Worker syntax·build·OBS budget·publish를 통과했다.
- 공개 Dashboard smoke는 기본 Speaker, YouTube→Setlink→Meloming, 한·영 reload, 320/375/768/1100px, 모바일 설정, 두 출력 버튼, 3px 금발 선, ntfy 요청 0·HTTP 오류 0을 통과했다. warm DCL `25.9ms`, long task 0, JS heap 약 `7.6MiB`였다. 공개 Dashboard chunk `Dashboard-C_ssb4Et.js`에서 `forceReset`과 번역 key `dashboard.playback.outputReset`도 확인했다.
- production Worker+공개 Pages 새-ID recovery smoke도 통과했다. fixture wall 오차 `176.3ms`, 최대 sample gap `93.6ms`, 16/16 marker, waiting/stalled/error/backward 0이었고, force reset 뒤 새 media는 paused·source 없음·autoplay false였다. 명시적 OBS 재선택 뒤에만 ready가 됐고 세션 종료·HTTP 410 fence까지 완료했다.
...
- release commit `e7a4acb1c5d6d466e3bbf93e661fc063dd69c38d`의 Pages workflow `29974560178`에서 build job `89103488646`과 deploy job `89103692362`가 성공했다. 내려받은 Actions artifact에서 manifest를 제외한 공개 파일 21개를 다시 수집해 SHA-256으로 비교했고 `21/21` exact match였다.
...
- release commit `e1cecf8ae9df9388306798a252345922a6a4c12e`의 Pages workflow `29975753233`, build job `89107013923`, deploy job `89107182934`, deployment `5566204012`가 성공했다. clean Ubuntu에서 735개 테스트·lint·Worker 문법·build·pseudo-locale·30곡 Blob 수명·OBS bundle·artifact upload·deploy를 모두 통과했다.
...
## 2026-07-23 (Codex) — v0.2.35 사용자 오류 번역 경계

- 대시보드와 재생 설정 화면에 남아 있던 `error.message` 직접 표시 22곳을 제거했다. 브라우저 영문 예외, WebSocket·lease·플레이어 내부 코드는 사용자 화면에 그대로 노출하지 않고, 현재 언어의 다음 행동 안내로 바꾼다.
- 준비되지 않은 곡, OBS용 로컬 파일 준비, OBS 경로 미확정처럼 사용자에게 원인을 구체적으로 알려도 되는 조건만 `UserActionError`의 semantic message key로 명시한다. 그 외 예외는 탐색·재생/일시정지·볼륨·정지·세션 종료별 안전한 fallback을 사용하며, 대기 중 언어가 바뀌어도 표시 시점의 locale로 번역한다.
- 한국어·영어에 탐색 실패, 재생 상태 변경 실패, 볼륨 변경 실패, 정지 요청 실패, 방송 세션 종료 실패 문구를 함께 추가했다. 모든 문구는 상태 코드 대신 현재 출력 확인·재시도·목록 보존 등 사용자의 다음 행동을 설명한다.
- live UI에서 `error.message`를 다시 사용하면 실패하는 source guard와 사용자 오류 경계 단위 테스트를 추가했다. 기존 소스 구조 테스트도 새 semantic error 계약을 검사하도록 갱신했다.
...
## 2026-07-23 (Codex) — v0.2.36 실제 OBS 확립 lease의 중복 후보 내성 인수

- Graphify에서 `Dashboard()`·`OnAirControlCoordinator`·`SessionRoom`·`obs-v2-external-cef-soak.mjs`를 따라, 새 후보 선택의 exact-one 조건과 이미 확립된 lease의 명령 대상 보존이 서로 다른 경계임을 다시 확인했다. 신규 활성화는 계속 후보 하나를 요구하지만 재생 중에는 정확한 기존 `playerInstanceId`와 `leaseEpoch`가 우선이다.
- 공개 Dashboard와 production Worker, 실제 OBS 30.2.0 CEF player로 A곡을 재생한 뒤 같은 세션의 두 번째 OBS-compatible player를 연결했다. player/eligible 후보가 2개가 되어도 lease target·audible lease·playing run·A곡 제목·`OBS 송출 중`이 유지됐다. 추가 player는 서버 `standby`, audio `paused=true`, source 없음, time 0이었고 media command 수신은 0건이었다.
- 추가 player가 들어오고 나가는 동안 Dashboard가 발행한 LOAD·PLAY·PAUSE·SEEK·VOLUME·STOP은 0건이었다. 추가 player를 닫자 같은 실제 OBS identity 하나로 돌아왔고 route나 current run을 다시 활성화·중지·재생하지 않았다.
- 같은 run에서 A 스킵 STOP `26` → strong-stop `30` → B LOAD `32`, B 자연 종료 ended `53` → STOP `54` → strong-stop `60` → C LOAD `62`, C 폐기 STOP `80` → strong-stop `84`까지 연속 통과했다. page/console error는 0건이고 종료 session은 HTTP 410이다.
- 첫 관찰 실행은 `players[].state`를 playback audibility와 혼동해 `ready !== audible`로 중단됐다. 이 필드는 route attachment이고 실제 audibility는 `lease.status`와 `confirmedPlayback.status`가 별도로 소유한다. 두 번째 실행은 숨은 로컬 DOM 진단 문자열을 서버 lease 권한처럼 강제해 timeout이 났다. 하네스는 서버 snapshot의 standby와 실제 media source·command 부재를 권위 있는 계약으로 사용하도록 교정했으며, 두 중단 session도 모두 HTTP 410으로 정리했다. 제품 코드는 바꾸지 않았다.
...
- 첫 Linux Actions 실행은 runner의 기본 언어가 영어라 첫 화면의 `Meloming`을 한국어 `멜로밍`으로 기대하던 Windows 로컬 가정을 드러내 배포 전에 중단됐다. smoke가 저장 locale을 명시적으로 한국어로 시작한 뒤 영어 전환·reload를 검사하도록 고쳐 OS 언어와 무관한 동일 계약으로 만들었다. 제품 locale 선택 로직은 바꾸지 않았다.
...
- 첫 Pages workflow `31303689723`은 세션 검증 뒤 prepare 인증 세대가 바뀌는 짧은
  구간에 요청 예약 집합이 비어 같은 곡의 prepare POST가 2번 발생해 배포 전에
  중단됐다. 세션 키 초기 채택 중에도 유지되는 곡별 in-flight 표식으로 기존 곡당
  1회 계약을 복구했다. 이 실패 실행에서는 Pages deploy와 공개 파일 변경이 발생하지 않았다.
